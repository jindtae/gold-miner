...
class Claw(pygame.sprite.Sprite):
    ...
    def rotate(self):
        self.image = pygame.transform.rotozoom(self.original_image, -self.angle,1) # 회전대상이미지, 회전각도, 이미지크기(scale)
...
class Gemstone(pygame.sprite.Sprite):
    ...
    def set_position(self,position,angle):
        ...
        to_x = r * math.cos(rad_angle) # 삼각형의밑변
...
def setup_gemstone():
    ...
    #작은 금
    small_gold = Gemstone(gemstone_images[0], (0,0),small_gold_price, small_gold_speed ) # 0번째 이미지를 200,300 d에 위치
    gemstone_group.add(small_gold) # 그룹의 추가
    #큰 금
    ...
    # 다이야몬드
    gemstone_group.add(Gemstone(gemstone_images[2], (900,420), diamond_price, diamond_speed ))
...
def display_game_over():
    ...
    txt_game_over = game_font.render(game_result,Ture,BLACK)
...
# 게임 오버 관련 변수
game_result = None # 개임 결과
total_time = 30 # 총 시간
start_ticks = pygame.time.get_ticks() # 현제 tick을 받아옴

# 게임 관련 변수
default_offset_x_claw = 40 # 중심점으로부터 집게까지의 기본 x 간격
to_x = 0 # x 좌표기준으로 집게 이미지를 이동시킬 값을 저장 변수
caught_gemstone = None # 집게를 뻗어서 잡은 보석 정보


# 속도변수
move_speed = 12 # 발사할 때 이동 스피드(x좌표 기주으로 증가되는 값)
...
BLACK = (0,0,0) #검은색
# 배경 이미지 불러오기
current_path = os.path.dirname(__file__) #현제 파일으 위치 반환
background = pygame.image.load(os.path.join(current_path,"backgorund.png"))

# 4개 보석 이미지 불러오기(작은금,큰금,돌,다이야몬드)
gemstone_images = [
...
claw = Claw(claw_image, (screen_width//2, 110)) #가로위치는 화면 기준으로 절반, 위에서110픽샐 위치

setup_gemstone() #게임에 원하는 마큼의 보석을 정의
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if  event.type == pygame.MOUSEBUTTONDOWN: # 마우스 버튼을 눌러 졌을 때 집게를 뻗음
             claw.set_direction(STOP)  # 좌우 멈춤
             to_x = move_speed    # move_speed 만큼 빠르게 뻗는 동작

    if claw.rect.left < 0 or claw.rect.right > screen_width or claw.rect.bottom > screen_height:
        to_x = -return_speed
    
    if claw.offset.x < default_offset_x_claw: # 원 위치에 오면
        to_x = 0
        claw.set_init_state() # 처음 상태로 되돌림

        if caught_gemstone: # 잡힌 보석이 있다면
            update_score(caught_gemstone.price) # 정수 업데이트 처리 (나중에)
    ...
    if not caught_gemstone: # 잡힌 보석이 없다면 충돌 체크
        for gemstone in gemstone_group:
            # if claw.rect.colliderect(gemstone.rect): # 직사각형 기준으로 츙돌처리
            if pygame.sprite.collide_mask(claw,gemstone): # 투명 영역은 제외하고 실제 이미지 존재하는 부분의 투명처리
                caught_gemstone = gemstone # 잡힌 보석
                to_x = -gemstone.speed # 잡힌 보석의 속도에 -값을 이동 속도로 설정
                break
    ...
    # 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 # 초 단위로 변경
    display_time(total_time - int(elapsed_time)) # 시간 표시

    # 마약에 시간이 0 이하이면 게임 종류
